...
UIChatbar::UIChatbar(Point<std::int16_t> pos)
{
    ...
    chat_field.set_enter_callback([this](const tiny_utf8::utf8_string& msg) {
        auto last = msg.find_last_not_of(U" "); // TODO: i18n
        if (last != tiny_utf8::utf8_string::npos) {
    ...
    chat_field.set_key_callback(KeyAction::UP, [this] {
        if (lastpos > 0) {
            --lastpos;
            chat_field.change_text(tiny_utf8::utf8_string{last_entered[lastpos]});
        }
    });
...
void UIChatbar::send_line(tiny_utf8::utf8_string&& line, LineType type)
{
    ++row_max;
    row_pos = row_max;
